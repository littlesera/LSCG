...
export class CursedItemModule extends BaseModule {
    ...
    load(): void {
        ...
        hookFunction("ElementButton.CreateForAsset", 1, ([idPrefix, asset, C, onClick, options, ...args], next) => {
            ...
            let isCursed = CursedKeywords.some(str => isPhraseInString(GetItemNameAndDescriptionConcat(asset) ?? "", str));
    ...
    CheckForCursedItems() {
        ...
        let allItems = Player.Appearance.filter(item => CursedKeywords.some(str => isPhraseInString(GetItemNameAndDescriptionConcat(item) ?? "", str)));
